...
class TwoLayerNet(object):
    ...
    def __init__(
        ...
        self.b1 = np.zeros_like(hidden_dim)
        self.w2 = weight_scale * np.random.randn(hidden_dim, num_classes)
        self.b2 = np.zeros_like(num_classes)
    ...
    def loss(self, X, y=None):
        ...
        h1 = X.dot(self.parameters['W1']) + self.parameters['b1']
        #h1 = np.maximum(0, h1)
        scores = h1.dot(self.parameters['W2']) + self.parameters['b2']
        ...
        prob0 = class_prob[0, y[0]]
        prob1 = class_prob[1, y[1]]
        prob2 = class_prob[2, y[2]]
        prob = prob0 + prob1 + prob2
        loss = np.sum(-np.log(prob))
        #loss = np.sum(-np.log(class_prob[range(self.N), y]))
        loss /= self.N
        loss += self.reg * (np.sum(self.w1 * self.w1) + np.sum(self.w2 * self.w2))
