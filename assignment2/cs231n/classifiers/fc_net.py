...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i, hidden_dim in enumerate(hidden_dims):
            rank = str(i + 1)
            self.params['W' + rank] = weight_scale * np.random.randn(pre_dim, hidden_dim)
    ...
    def loss(self, X, y=None):
        ...
        if self.normalization is None:
            for i in range(self.num_layers - 1):
                ...
                if self.use_dropout:
                    layer_input, dt_cache = dropout_forward(layer_input, self.dropout_param)
        ...
        scores, fc_cache = affine_forward(layer_input, self.params['W%s' % self.num_layers],
        ...
        if self.normalization is None:
            ...
            for i in range(self.num_layers - 1, 0, -1):
                if self.use_dropout:
                    dout = dropout_backward(dout, caches[i - 1][-1])
                dout, dw, db, dgamma, dbeta = backward_func(dout, caches[i - 1][0])
