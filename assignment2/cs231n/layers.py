...
def conv_backward_naive(dout, cache):
    ...
    H_ap = 1 + (H + 2 * pad - HH) // stride
    W_ap = 1 + (W + 2 * pad - WW) // stride
    x_padding = np.pad(x, ((0, 0), (0, 0), (pad, pad), (pad, pad)), mode='constant', constant_values=0)
    ...
    for f in range(F):
        for j in range(H_ap):
            for k in range(W_ap):
                ...
                dw[f] += np.sum(
                    x_padding[:, :, j * stride:j * stride + HH, k * stride:k * stride + WW] * dout[:, f, j, k][:, None,
                                                                                              None, None], axis=0)
                dx_padding[:, :, j * stride:j * stride + HH, k * stride:k * stride + WW] += w[f] * dout[:, f, j, k][:,
                                                                                                   None, None, None]
...
def spatial_groupnorm_backward(dout, cache):
    ...
    x_group = x.reshape((N, G, C // G, H, W))
    dx_var = np.sum(dx_groupnorm * -1.0 / 2 * (x_group - x_mean) / (x_var + eps) ** (3.0 / 2), axis=(2, 3, 4),
                    keepdims=True)

    m = C // G * H * W

    dx_mean = np.sum(-dx_groupnorm / np.sqrt(x_var + eps), axis=(2, 3, 4), keepdims=True) + \
